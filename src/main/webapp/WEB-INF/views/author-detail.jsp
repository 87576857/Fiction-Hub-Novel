...
<!-- 주소: author-detail.jsp -->
<main class="container-xl py-4 px-3" style="display:block; max-width:760px;">
  <button class="btn-back" onclick="location.href='${pageContext.request.contextPath}/author'"><i class="bi bi-arrow-left"></i> 목록으로</button>
  ...
  <c:choose>
    ...
    <c:otherwise>
      ...
      <div class="board-card mb-3" style="background:var(--accent-author);">
      ...
      <p class="mb-4" style="font-size:14px;line-height:1.8;background:var(--muted-bg);border-radius:1rem;padding:16px 18px; white-space:pre-line;"><c:out value="${post.content}"/></p>

      <c:if test="${not empty sessionScope.loginUser and sessionScope.loginUser.userId == post.userId}">
        <div class="text-end mb-4">
          <button type="button" class="btn-outline-custom" onclick="location.href='${pageContext.request.contextPath}/author/${post.postId}/edit'">수정</button>
        </div>
      </c:if>
      ...
      <c:choose>
        <c:when test="${empty comments}">
          <p class="text-muted" style="font-size:14px;">아직 댓글이 없습니다.</p>
        </c:when>
        <c:otherwise>
      ...
      <form method="post" action="${pageContext.request.contextPath}/author/${post.postId}/comment" class="muted-panel mt-4">
